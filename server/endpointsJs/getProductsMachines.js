...
async function getProductsMachinesHandler(req, res) {
    try {
        var data = await Product.find({type: "fitnessMachine"}).sort({ uploadDate: 'desc' }).lean();
...
module.exports = getProductsMachinesHandler;